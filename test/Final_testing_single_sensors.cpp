...
void setup() {
    ...
    Serial.println("\n********** RUNNING SELF-TEST **********");
    int flag  = check_self_test();;

    while (flag == 0){
        flag  =  check_self_test();
    }
...
void loop() {
    ...
    if (rslt == BMM350_OK) {
        ...
        Serial.print("X: "); Serial.print(mag_temp_data.x, 2);
        Serial.print("  Y: "); Serial.print(mag_temp_data.y, 2);
        Serial.print("  Z: "); Serial.print(mag_temp_data.z, 2);
        Serial.print("  Temp: "); Serial.println(mag_temp_data.temperature, 2);
    } else {
        Serial.println("Failed to read data!");
    }

    if (counter == 100) {
        Serial.println("\nChecking sensor is working properly...");
        int flag = 0;
        while(flag == 0) {
            flag = check_self_test();
        }
