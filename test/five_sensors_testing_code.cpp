...
void selectMuxChannel(uint8_t channel) {
    ...
    delayMicroseconds(100); // Small delay for mux switching
}
...
void setup() {
    ...
    for (uint8_t i = 0; i < NUM_SENSORS; i++) {
        Serial.printf("\n--- Initializing Sensor %d (Channel %d) ---\n", i, SENSOR_CHANNELS[i]);
        if (!initSensor(i)) {
    ...
    // Read initial samples from all sensors
    Serial.println("\n********** INITIAL READINGS **********");
    ...
    Serial.println("\n********** RUNNING SELF-TEST ON ALL SENSORS **********");
...
void loop() {
    ...
    // Print CSV format: Timestamp, S0_X, S0_Y, S0_Z, S0_T, S1_X, ...
